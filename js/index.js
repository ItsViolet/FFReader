/*jshint esversion: 11 */
let appName = "FFReader";
let appVersion = "beta_0.1";
...
app = new Vue({
  ...
  async mounted() {
    ...
    this.searchArea = document.getElementById("searchRes");
  ...
  methods: {
    ...
    addNewNovelAO3(id) {
      console.log("hi");
      let t_url = "https://archiveofourown.org/works/"+id;
      if (this.iframe_working) {
        msgBox("Renderer busy, please wait");
        return;
      }
    ...
    async search() {
      let t_url = this.search_query.trim();
      // if (this.iframe_working) {
      //   msgBox("Renderer busy, please wait");
      //   return;
      // }
      ...
      //   return;
      // }
      fetch("http://localhost:9877/search?q="+t_url+"&p=1").then((res) => {
        return res.json();
      }).then((r) => {
        let e = "";
      ...
      for (let i = 0; i < chapter_urls.length; i++) {
        ...
        if (this.r_chapters[i] == null) {
          ...
          if (chapter_file_exist) {
            ...
            if(!this.is_downloading){
            setTimeout(async () => {
              this.downloadNovel(this.r_novel);
            }, 2000);}
          }
        }
      }
    ...
    async semiLoadChapters() {
      this.loading_chapters = true;
      console.log(this.r_novel);
      this.r_chapters = Array.apply(
        null,
        Array(this.r_novel.ChapterURLs.length)
      ).fill(null);
